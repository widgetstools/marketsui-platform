...
export interface TickerItem { label:string; value:string; change:string; up:boolean; }
export interface Order {
  id:string; time:string; bond:string; side:string; qty:string;
  type:string; px:number; ytm:number; status:'Filled'|'Partial'|'Pending'|'Cancelled'; filled:string;
}
export interface ResearchNote {
  id:string; date:string; author:string; ticker:string; title:string;
  body:string; rating:string; target:number|null; prev:number|null; sector:string;
}
export interface RiskPosition { book:string; bonds:number; mv:number; dv01:number; oas:number; pnl:number; }
export interface MarketIndex { name:string; val:number; chg:number; ytd:string; }
...
// ── VaR trend ──
export const VAR_DATA = Array.from({length:30},(_,i)=>({day:`D-${30-i}`,var:-(Math.random()*180+120)}));
...
export const BUCKET_RANGES: [number,number][] = [[0,1],[1,3],[3,5],[5,7],[7,10],[10,50]];
...
export const SECTOR_COLORS=['#1e90ff','#00bcd4','#c084fc','var(--bn-green)','#f0b90b','#f59e0b'];

// ── Historical OAS ──
export const HIST_OAS=Array.from({length:60},(_,i)=>{const d=new Date(2026,3,4);d.setDate(d.getDate()-59+i);return{date:`${d.getMonth()+1}/${d.getDate()}`,ig:+(52+Math.sin(i/8)*8+(Math.random()-.5)*3).toFixed(1),hy:+(340+Math.sin(i/6)*25+(Math.random()-.5)*8).toFixed(1)};});
...
export const INTRADAY:Record<string,{t:string;v:number}[]>={'UST 10Y':makeIntraday(4.27),'CDX IG':makeIntraday(52.9),'CDX HY':makeIntraday(339.6),'SOFR':makeIntraday(5.33)};

@Injectable({ providedIn: 'root' })
export class TradingDataService {
  getBonds(): Bond[] { return BONDS; }
  getOrders(): Order[] { return INITIAL_ORDERS; }
  getTickerStrip(): TickerItem[] { return TICKER_STRIP; }
}
