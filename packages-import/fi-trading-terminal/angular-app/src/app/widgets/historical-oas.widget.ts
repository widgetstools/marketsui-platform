  ...
  template: `
    <div style="display:flex;flex-direction:column;height:100%;background:var(--bn-bg1);overflow:hidden">
      <div style="display:flex;justify-content:flex-end;padding:4px 10px;flex-shrink:0">
        <button *ngFor="let p of periods" (click)="period=p;draw()" class="font-mono-fi"
          ...
          [style.color]="period===p?'var(--bn-t0)':'var(--bn-t1)'">{{p}}</button>
      </div>
...
export class HistoricalOasWidget implements AfterViewInit, OnDestroy {
  ...
  draw() {
    ...
    const data = HIST_OAS;
    const n = data.length;
    const igVals = data.map(d => d.ig), hyVals = data.map(d => d.hy);
    const minIg = Math.min(...igVals) * 0.98, maxIg = Math.max(...igVals) * 1.02;
    const minHy = Math.min(...hyVals) * 0.98, maxHy = Math.max(...hyVals) * 1.02;
    ...
    igGrad.addColorStop(0, 'rgba(30,144,255,0.15)'); igGrad.addColorStop(1, 'rgba(30,144,255,0)');
    ctx.beginPath();
    data.forEach((d, i) => { i === 0 ? ctx.moveTo(xOf(i), yIg(d.ig)) : ctx.lineTo(xOf(i), yIg(d.ig)); });
    ctx.lineTo(xOf(n - 1), H - pad.b); ctx.lineTo(pad.l, H - pad.b); ctx.closePath();
    ctx.fillStyle = igGrad; ctx.fill();
    ctx.beginPath(); ctx.strokeStyle = '#1e90ff'; ctx.lineWidth = 1.5;
    data.forEach((d, i) => { i === 0 ? ctx.moveTo(xOf(i), yIg(d.ig)) : ctx.lineTo(xOf(i), yIg(d.ig)); });
    ctx.stroke();

    // HY area
    const hyGrad = ctx.createLinearGradient(0, pad.t, 0, H - pad.b);
    hyGrad.addColorStop(0, 'rgba(248,113,113,0.1)'); hyGrad.addColorStop(1, 'rgba(248,113,113,0)');
    ctx.beginPath();
    data.forEach((d, i) => { i === 0 ? ctx.moveTo(xOf(i), yHy(d.hy)) : ctx.lineTo(xOf(i), yHy(d.hy)); });
    ctx.lineTo(xOf(n - 1), H - pad.b); ctx.lineTo(pad.l, H - pad.b); ctx.closePath();
    ctx.fillStyle = hyGrad; ctx.fill();
    ctx.beginPath(); ctx.strokeStyle = g('--bn-red'); ctx.lineWidth = 1.5;
    data.forEach((d, i) => { i === 0 ? ctx.moveTo(xOf(i), yHy(d.hy)) : ctx.lineTo(xOf(i), yHy(d.hy)); });
    ctx.stroke();

    // Legend
    ctx.textAlign = 'left';
    [['CDX IG', '#1e90ff'], ['CDX HY', g('--bn-red')]].forEach(([label, color], idx) => {
      ctx.fillStyle = color as string; ctx.fillRect(pad.l + idx * 70, 2, 12, 3);
      ctx.fillStyle = g('--bn-t2'); ctx.font = '8px JetBrains Mono,monospace';
