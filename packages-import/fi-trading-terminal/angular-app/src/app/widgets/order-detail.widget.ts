    ...
    >
      <ng-container *ngIf="shared.selectedOrder() as sel">
        ...
        >
          ...
          >
            <div
              style="font-family:JetBrains Mono,monospace;font-weight:700;font-size:11px;color:#00bcd4;margin-bottom:5px"
            >
              {{ sel.bond }}
            </div>
            <span
          ...
          <button
            *ngIf="sel.status === 'Pending' || sel.status === 'Partial'"
            style="width:100%;padding:7px;border-radius:3px;border:1px solid rgba(248,113,113,0.3);background:rgba(248,113,113,0.1);color:var(--bn-red);font-family:JetBrains Mono,monospace;font-weight:700;font-size:11px;cursor:pointer;margin-top:4px"
          >
            CANCEL ORDER
          </button>
        </div>
