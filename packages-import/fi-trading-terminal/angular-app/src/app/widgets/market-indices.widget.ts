        ...
        <table style="width:100%;border-collapse:collapse">
          <thead><tr style="background:var(--bn-bg2);position:sticky;top:0">
            <th *ngFor="let h of headers" style="font-size:11px;color:var(--bn-t1);padding:5px 10px;border-bottom:1px solid var(--bn-border);font-weight:400;letter-spacing:0.04em"
              [style.textAlign]="h==='INDEX'?'left':'right'">{{h}}</th>
          </tr></thead>
          <tbody>
            <tr *ngFor="let idx of indices" style="border-bottom:1px solid rgba(43,49,57,0.5)">
              <td style="padding:6px 10px;font-size:11px;color:var(--bn-t0);font-family:JetBrains Mono,monospace">{{idx.name}}</td>
              <td style="padding:6px 10px;font-size:11px;font-family:JetBrains Mono,monospace;color:var(--bn-t0);text-align:right">{{idx.val.toFixed(2)}}</td>
              <td style="padding:6px 10px;font-size:11px;font-family:JetBrains Mono,monospace;text-align:right"
                [style.color]="idx.chg>=0?'var(--bn-green)':'var(--bn-red)'">{{idx.chg>=0?'+':''}}{{idx.chg.toFixed(2)}}</td>
              <td style="padding:6px 10px;font-size:11px;font-family:JetBrains Mono,monospace;text-align:right"
                [style.color]="idx.ytd.startsWith('+')?'var(--bn-green)':'var(--bn-red)'">{{idx.ytd}}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export class MarketIndicesWidget implements OnInit, OnDestroy {
  ...
  ngOnDestroy() { if (this.tickId) clearInterval(this.tickId); }
}
