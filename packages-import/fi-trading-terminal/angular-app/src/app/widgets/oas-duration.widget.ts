...
const RTG_COLOR: Record<string, string> = { aaa: '#1e90ff', aa: '#00bcd4', a: '#2dd4bf', bbb: '#f0b90b', hy: '#f87171' };
...
@Component({
  ...
  template: `
    <div style="flex:1;position:relative;background:var(--bn-bg1)"><canvas #canvas style="width:100%;height:100%;display:block"></canvas></div>
    <div style="display:flex;gap:10px;padding:4px 14px 8px;flex-shrink:0;background:var(--bn-bg1)">
      <div *ngFor="let item of legend" style="display:flex;align-items:center;gap:4px">
        ...
        <span style="font-size:9px;color:var(--bn-t2);font-family:JetBrains Mono,monospace">{{item.label}}</span>
      </div>
    </div>
  `,
})
export class OasDurationWidget implements AfterViewInit, OnDestroy {
  ...
  private draw() {
    ...
    SCATTER_DATA.forEach(d => {
      ...
      ctx.beginPath(); ctx.arc(xOf(d.x), yOf(d.y), r, 0, Math.PI * 2);
      ctx.fillStyle = color + 'bf'; ctx.fill();
