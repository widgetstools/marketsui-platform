...
@Component({
  ...
  template: `<div style="flex:1;position:relative;background:var(--bn-bg1)"><canvas #canvas style="width:100%;height:100%;display:block"></canvas></div>`,
})
export class YieldCurveWidget implements AfterViewInit, OnDestroy {
  ...
  private draw() {
    ...
    const minV = Math.min(...allVals) * 0.99, maxV = Math.max(...allVals) * 1.01;
    const n = data.length;
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      ctx.fillStyle = g('--bn-t2'); ctx.font = '9px JetBrains Mono,monospace'; ctx.textAlign = 'right';
      ctx.fillText(val.toFixed(2), pad.l - 4, y + 3);
    }
    // X labels
    data.forEach((d, i) => {
      ctx.fillStyle = g('--bn-t2'); ctx.font = '9px JetBrains Mono,monospace'; ctx.textAlign = 'center';
      ctx.fillText(d.tenor, xOf(i), H - pad.b + 14);
    });

    // Lines
    const drawLine = (key: 'today'|'week'|'month', color: string, width: number, dash: number[]) => {
      ctx.beginPath(); ctx.strokeStyle = color; ctx.lineWidth = width; ctx.setLineDash(dash);
      data.forEach((d, i) => { i === 0 ? ctx.moveTo(xOf(i), yOf(d[key])) : ctx.lineTo(xOf(i), yOf(d[key])); });
      ctx.stroke(); ctx.setLineDash([]);
    };
    drawLine('month', g('--bn-bg2'), 1, [2, 4]);
    drawLine('week', g('--bn-border'), 1.2, [4, 4]);
    drawLine('today', '#1e90ff', 2, []);

    // Today dots
    data.forEach((d, i) => {
      ctx.beginPath(); ctx.arc(xOf(i), yOf(d.today), 2.5, 0, Math.PI * 2);
      ctx.fillStyle = '#1e90ff'; ctx.fill();
    });

    // Legend
    ctx.textAlign = 'left';
    [['Today', '#1e90ff'], ['-1 Week', g('--bn-border')], ['-1 Month', g('--bn-bg2')]].forEach(([label, color], idx) => {
      ctx.fillStyle = color as string; ctx.fillRect(pad.l + idx * 80, 4, 12, 3);
