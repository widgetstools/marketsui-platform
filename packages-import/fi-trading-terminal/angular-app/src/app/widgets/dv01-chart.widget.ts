...
const BAR_COLORS = ['#1e90ff', 'var(--bn-red)', '#1e90ff', '#00bcd4', '#c084fc'];
...
export class Dv01ChartWidget implements AfterViewInit, OnDestroy {
  ...
  private draw() {
    ...
    // Grid
    ctx.strokeStyle = getComputedStyle(document.documentElement).getPropertyValue('--bn-bg2').trim();
    ctx.lineWidth = 0.5;
    for (let i = 1; i <= 4; i++) {
    ...
    DV01_DATA.forEach((d, i) => {
      ...
      ctx.fillText(d.name.replace('CREDIT-', '').replace('RATES-', ''), x + barW / 2, H - pad.b + 14);
    });
  }
}
