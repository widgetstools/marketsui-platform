    ...
    >
      ...
      <div style="flex:1;overflow-y:auto">
        <table style="width:100%;border-collapse:collapse">
          <thead>
            <tr style="background:var(--bn-bg2);position:sticky;top:0;z-index:1">
              <th
                *ngFor="let h of headers"
                ...
                [style.textAlign]="h === 'BOND' || h === 'TIME' ? 'left' : 'right'"
              >
                {{ h }}
              </th>
            </tr>
          ...
          <tbody>
            <tr
              *ngFor="let o of filteredOrders"
              (click)="shared.selectedOrder.set(o)"
              style="border-bottom:1px solid rgba(43,49,57,0.5);cursor:pointer"
              [style.background]="
                shared.selectedOrder()?.id === o.id ? 'var(--bn-bg2)' : 'transparent'
              "
            >
              <td
                style="padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:9px;color:var(--bn-t2)"
              >
                {{ o.time }}
              </td>
              <td
                style="padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:11px;color:#00bcd4"
              >
                {{ o.bond }}
              </td>
              ...
              <td
                style="padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:9px;color:var(--bn-t1);text-align:right"
              >
                {{ o.type }}
              </td>
              <td
                style="padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:11px;color:var(--bn-t0);text-align:right"
              >
                {{ o.qty }}
              </td>
              <td
                style="padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:11px;text-align:right"
                [style.color]="o.filled === o.qty ? 'var(--bn-green)' : '#f0b90b'"
              >
                {{ o.filled }}
              </td>
              <td
              ...
              <td
                style="padding:5px 10px;font-family:JetBrains Mono,monospace;font-size:11px;color:var(--bn-t1);text-align:right"
              >
                {{ o.ytm > 0 ? o.ytm.toFixed(2) + '%' : '---' }}
              </td>
