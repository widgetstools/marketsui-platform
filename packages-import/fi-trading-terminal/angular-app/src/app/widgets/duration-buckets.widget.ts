        ...
        <table style="width:100%;border-collapse:separate;border-spacing:2px">
          <thead>
            <tr>
              ...
              <th
                style="font-size:9px;color:var(--bn-t2);text-align:right;padding:4px 6px;font-weight:400"
              >
                Bonds
              </th>
              <th
              ...
              <th
                style="font-size:9px;color:var(--bn-t2);text-align:left;padding:4px 6px;font-weight:400"
              >
                Distribution
              </th>
            </tr>
            ...
            <tr *ngFor="let d of buckets">
              <td style="padding:6px 8px;border-radius:3px;background:var(--bn-bg2)">
                <span
                  ...
                  style="font-size:11px;color:var(--bn-t0)"
                  >{{ d.label }}</span
                >
              </td>
              <td
                [style.background]="heatBg(d.count, 5, '#c084fc')"
                style="padding:6px 6px;border-radius:3px;text-align:right"
              >
                <span
                  class="font-mono-fi font-semibold"
                  style="font-size:11px;color:var(--bn-t0)"
              ...
              <td
                [style.background]="heatBg(d.dv01, 3000, '#1e90ff')"
                style="padding:6px 6px;border-radius:3px;text-align:right"
              >
                <span class="font-mono-fi font-semibold" style="font-size:9px;color:var(--bn-t0)"
                  >\${{ (d.dv01 / 1000).toFixed(1) }}K</span
                >
              </td>
              <td
                [style.background]="heatBg(getRiskPct(d), 35, '#1e90ff')"
                style="padding:6px 6px;border-radius:3px;text-align:right"
              >
                <span class="font-mono-fi" style="font-size:9px;color:var(--bn-t0)"
                  >{{ getRiskPct(d).toFixed(0) }}%</span
                >
              </td>
              <td
                [style.background]="heatBg(d.avgOas, 150, '#f0b90b')"
                style="padding:6px 6px;border-radius:3px;text-align:right"
              >
                <span class="font-mono-fi" style="font-size:9px;color:var(--bn-t0)">{{
                  d.avgOas > 0 ? '+' + d.avgOas + 'bp' : '---'
                }}</span>
              </td>
              <td style="padding:6px 6px;border-radius:3px">
                <div style="display:flex;align-items:center;gap:4px">
                  <div
                    style="flex:1;height:6px;border-radius:3px;background:var(--bn-bg3);overflow:hidden"
                  >
                    <div
                      [style.width.%]="getRiskPct(d) * 2.8"
                      style="height:100%;border-radius:3px;opacity:0.7"
                      [style.background]="
                        d.avgOas > 80 ? '#f0b90b' : d.avgOas > 30 ? '#00bcd4' : '#1e90ff'
                      "
                    ></div>
                  </div>
                </div>
...
export class DurationBucketsWidget {
  ...
  summary = [
    ...
    {
      l: 'Wt Avg OAS',
      v: `+${Math.round(BUCKET_DETAIL.reduce((a, d) => a + d.avgOas * d.count, 0) / BUCKET_DETAIL.reduce((a, d) => a + d.count, 0))}bp`,
      c: '#f0b90b',
    },
  ];

  getRiskPct(d: any) {
    return this.totalDv01 > 0 ? (d.dv01 / this.totalDv01) * 100 : 0;
  }
  heatBg = heatBg;
}
