...
@Component({
  ...
  template: `
    <div style="display:grid;grid-template-columns:repeat(6,1fr);height:100%;overflow:hidden">
      <div *ngFor="let k of kpis; let i = index" style="background:var(--bn-bg1);padding:10px 14px;display:flex;flex-direction:column;justify-content:center"
        [style.borderRight]="i<5?'1px solid var(--bn-border)':'none'">
        <div style="font-size:11px;color:var(--bn-t1);margin-bottom:3px;text-transform:uppercase;letter-spacing:0.05em">{{k.label}}</div>
        <div style="font-size:18px;font-weight:600;font-family:JetBrains Mono,monospace" [style.color]="k.color">{{k.val}}</div>
        <div style="font-size:9px;color:var(--bn-t2);margin-top:2px;font-family:JetBrains Mono,monospace">{{k.sub}}</div>
      </div>
    </div>
  `,
})
