...
const COLS = [
  ...
  { key: 'totalDv01', label: 'DV01', fmt: (v: number) => `$${(v / 1000).toFixed(1)}K`, color: '#1e90ff' },
...
COLS.forEach(c => { colMaxes[c.key] = Math.max(...SECTOR_HEATMAP.map(s => (s as any)[c.key] as number)); });
    ...
    <div style="display:flex;flex-direction:column;height:100%;background:var(--bn-bg1);overflow:hidden">
      <div style="flex:1;overflow:auto;padding:6px">
        <table style="width:100%;border-collapse:separate;border-spacing:2px">
          <thead><tr>
            <th style="font-size:9px;color:var(--bn-t2);text-align:left;padding:4px 8px;font-weight:400">Sector</th>
            <th *ngFor="let c of cols" style="font-size:9px;color:var(--bn-t2);text-align:right;padding:4px 6px;font-weight:400;white-space:nowrap">{{c.label}}</th>
          </tr></thead>
          <tbody>
            <tr *ngFor="let s of sectors; let i = index">
              <td style="padding:5px 8px;border-radius:3px">
                <div style="display:flex;align-items:center;gap:5px">
                  <div [style.background]="sectorColors[i]" style="width:3px;height:16px;border-radius:2px;flex-shrink:0"></div>
                  <span style="font-size:9px;color:var(--bn-t0);white-space:nowrap">{{s.sector}}</span>
                </div>
              </td>
              <td *ngFor="let c of cols" [style.background]="getHeatBg(getVal(s,c.key),c.key,c.color)" style="padding:5px 6px;border-radius:3px;text-align:right">
                <span class="font-mono-fi font-semibold" style="font-size:9px;color:var(--bn-t0)">{{c.fmt(getVal(s,c.key))}}</span>
              </td>
            </tr>
          </tbody>
        </table>
...
export class SectorAllocationWidget {
  ...
  getVal(s: any, key: string): number { return s[key] as number; }
  getHeatBg(value: number, key: string, color: string): string { return heatBg(value, colMaxes[key], color); }
}
