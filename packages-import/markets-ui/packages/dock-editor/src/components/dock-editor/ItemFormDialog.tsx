            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
                ...
                <label
                  ...
                  style={{
                    width: 28, height: 28, borderRadius: "var(--de-radius-sm)", cursor: "pointer",
                    border: iconColor && !PREDEFINED_COLORS.some(c => c.hex === iconColor)
                      ? "2px solid var(--de-accent)"
                      : "1px solid var(--de-border-strong)",
                    background: iconColor && !PREDEFINED_COLORS.some(c => c.hex === iconColor)
                      ? iconColor
                      : "var(--de-bg-surface)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    transition: "all 0.12s", flexShrink: 0, overflow: "hidden",
                    boxShadow: iconColor && !PREDEFINED_COLORS.some(c => c.hex === iconColor)
                      ? "0 0 0 2px var(--de-accent-dim)"
                      : "none",
                  }}
                >
                  {/* Show a palette icon when no custom color is active */}
                  {(!iconColor || PREDEFINED_COLORS.some(c => c.hex === iconColor)) && (
